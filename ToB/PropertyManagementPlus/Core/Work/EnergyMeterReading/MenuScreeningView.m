...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        self.oneLinkageButton.frame = CGRectMake(2 * kWidth/3, 0,  kWidth/3, Height);
        [self setUpButton:self.oneLinkageButton withText:@"状态"];
        ...
        self.twoLinkageButton.frame = CGRectMake(kWidth/3, 0, kWidth/3, Height);
        [self setUpButton:self.twoLinkageButton withText:@"类型"];
        ...
        self.threeLinkageButton.frame = CGRectMake(0, 0, kWidth/3, Height);
        [self setUpButton:self.threeLinkageButton withText:@"位置"];
...
-(void)clickButton:(UIButton *)button{
    ...
    if (button == self.oneLinkageButton) {
        ...
        [self.oneLinkageDropMenu creatDropView:self withShowTableNum:1 withData:self.sortsArr selectIndex:1];
        ...
        [self.twoLinkageDropMenu creatDropView:self withShowTableNum:1 withData:self.categoriesArr selectIndex:2];
        ...
        [self.threeLinkageDropMenu creatDropView:self withShowTableNum:3 withData:self.addressArr selectIndex:3];
    }
}
...
-(void)setUpButton:(UIButton *)button withText:(NSString *)str{
    ...
    verticalLine.frame = CGRectMake(button.frame.size.width - 0.5, 3, 0.5, Height);
}
...
-(NSArray *)addressArr{
    ...
    if (_addressArr == nil) {
        ...
        NSDictionary *dic = [[NSUserDefaults standardUserDefaults] objectForKey:@"HouseStructureDataSynchronization"];
