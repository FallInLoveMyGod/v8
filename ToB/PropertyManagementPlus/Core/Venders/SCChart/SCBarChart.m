...
-(void)strokeChart
{
    NSArray *arr = @[@"#DFDFDF",@"#DDDDDD",@"#33B5E5",@"#AA66CC",@"#99CC00",@"#FFBB33",@"#FF4444"];
    CGFloat chartCavanHeight = self.frame.size.height - UULabelHeight*3;
    ...
    for (int i=0; i<_yValues.count; i++) {
        ...
        for (int j=0; j<childAry.count; j++) {
            ...
            //            bar.barColor = [_colors objectAtIndex:<#(NSUInteger)#>]
            
            NSInteger index = arc4random() % 7;
            UIColor *color = [UIColor colorwithHexString:arr[index]];
            bar.barColor = color;
            bar.grade = grade;
            ...
            // 数量标注
            UILabel *lab = [[UILabel alloc] initWithFrame:CGRectMake(-15, bar.bounds.size.height * (1 - grade) , _xLabelWidth + 10, 25)];
            if (grade < 0.07) {
                lab.frame = CGRectMake(-15, bar.bounds.size.height * (1 - grade) - 25 , _xLabelWidth + 10, 25);
            }
