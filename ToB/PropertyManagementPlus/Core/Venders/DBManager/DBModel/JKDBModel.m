...
{
    NSDictionary *exceptValueKeys = [NSDictionary dictionaryWithObjectsAndKeys:@[@"Contents"],@"EquipmentPatrolGroupModel", nil];
    ...
    for (i = 0; i < outCount; i++) {
        ...
        NSString *tableName = [NSStringFromClass(self.class) componentsSeparatedByString:@"."].lastObject;
        NSArray *excepts = exceptValueKeys[tableName];
        if ([excepts containsObject:propertyName]) {
            continue;
        }
...
+ (BOOL)isExistInTable
{
    ...
    [jkDB.dbQueue inDatabase:^(FMDatabase *db) {
        ...
        NSArray *nameArray = [tableName componentsSeparatedByString:@"."];
        tableName = nameArray[nameArray.count - 1];
...
+ (BOOL)upgrateDataBase:(NSArray *)colums {
    ...
    [jkDB.dbQueue inTransaction:^(FMDatabase *db, BOOL *rollback) {
        ...
        NSArray *nameArray = [tableName componentsSeparatedByString:@"."];
        tableName = nameArray[nameArray.count - 1];
        NSString *queryExecute = [NSString stringWithFormat:@"SELECT sql FROM sqlite_master WHERE type = 'table' AND tbl_name = '%@'",tableName];
        FMResultSet *rs = [db executeQuery:queryExecute];
        ...
        if (tableIsExistus && tabCreateSql) {
            for (NSString *column in colums) {
                ...
                NSRange range1 = [tabCreateSql rangeOfString: column];
                if (range1.length < 1) {
                    NSString *sql = [NSString stringWithFormat:@"ALTER TABLE %@ ADD %@ varchar(256); ", tableName,column];
...
+ (NSArray *)findAllExceptions:(NSArray *)exceptions {
    NSLog(@"jkdb---%s",__func__);
    JKDBHelper *jkDB = [JKDBHelper shareInstance];
    NSMutableArray *users = [NSMutableArray array];
    [jkDB.dbQueue inDatabase:^(FMDatabase *db) {
        ...
        while ([resultSet next]) {
            ...
            for (int i=0; i< model.columeNames.count; i++) {
                NSString *columeName = [model.columeNames objectAtIndex:i];
                if ([exceptions containsObject:columeName]) {
                    continue;
                }
                NSString *columeType = [model.columeTypes objectAtIndex:i];
                if ([columeType isEqualToString:SQLTEXT]) {
        ...
        while ([resultSet next]) {
            ...
            for (int i=0; i< model.columeNames.count; i++) {
                ...
                if ([columeType isEqualToString:SQLTEXT]) {
                    ...
                    if ([columeName isEqualToString:@"files"] && [columeType isEqualToString:@"REAL"] && [model isKindOfClass:[GetRepaireListModel class]]) {
                        ...
                        @try {
                            ...
                            //非标准json，需要转换
                            NSString * responseString = [resultSet stringForColumn:columeName];
                            
                            NSArray *occurrenceArray = @[@"\r\n",@"\n",@"\t",@"(",@")",@"=",@";",@"BussinessType",@"Url"];
                            NSArray *replaceArray = @[@"",@"",@"",@"[",@"]",@":",@",",@"\"BussinessType\"",@"\"Url\""];
                            
                            for (int i = 0; i < occurrenceArray.count; i ++) {
                                responseString = [responseString stringByReplacingOccurrencesOfString:occurrenceArray[i] withString:replaceArray[i]];
                            }
                            
                            NSData* data = [responseString dataUsingEncoding:NSUTF8StringEncoding];
                            __autoreleasing NSError* error = nil;
                            
                            [model setValue:[NSArray arrayWithArray:[NSJSONSerialization JSONObjectWithData:data options:NSJSONReadingMutableLeaves error:&error]] forKey:columeName];
...
+ (NSArray *)findByCriteriaSQL:(NSString *)criteriaSQL {
