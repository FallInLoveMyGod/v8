...
{
    if (self.frame.size.height<250 || self.frame.size.width<[[UIScreen mainScreen] bounds].size.width)
        self.frame = CGRectMake(self.frame.origin.x, self.frame.origin.y, [[UIScreen mainScreen] bounds].size.width, 250);
    ...
    CGFloat space = 10;
    CGFloat width = (self.frame.size.width - space * 4) / 3;
    if (!cancelButton) {
        
        cancelButton = [self createButtonWithFrame:CGRectMake(space, 0, width, 40) title:@"取消" titleColor:[UIColor blackColor] font:[UIFont systemFontOfSize:16] backGroundColor:[UIColor groupTableViewBackgroundColor] target:self action:@selector(cancelButtonClick)];
    ...
    if (!resetButton) {
        ...
        resetButton = [self createButtonWithFrame:CGRectMake(cancelButton.frame.origin.x + cancelButton.frame.size.width + space, 0, width, 40) title:@"重置" titleColor:[UIColor blackColor] font:[UIFont systemFontOfSize:16] backGroundColor:[UIColor groupTableViewBackgroundColor] target:self action:@selector(resetButtonClick)];
    ...
    if (!confirmButton) {
        ...
        confirmButton = [self createButtonWithFrame:CGRectMake(resetButton.frame.origin.x + resetButton.frame.size.width + space, 0, width, 40) title:@"确定" titleColor:[UIColor blackColor] font:[UIFont systemFontOfSize:16] backGroundColor:[UIColor groupTableViewBackgroundColor] target:self action:@selector(confirmButtonClick)];
...
- (void)creatValuePointXs:(NSArray *)xArr withNames:(NSArray *)names
{
    ...
    for (int i=0; i<xArr.count; i++) {
        NSInteger pointX = [xArr[i] intValue] + 20;
        ...
        [self addLabelWithNames:names[i] withPointX:pointX];
    }
     */
}
...
{
    UILabel *label = [[UILabel alloc]initWithFrame:CGRectMake(point_x, 99 + 16, 20, 20)];
    label.text = name;
...
//UI控制
- (UIButton *)createButtonWithFrame:(CGRect)frame title:(NSString *)title titleColor:(UIColor *)titleColor font:(UIFont *)font backGroundColor:(UIColor *)backGroundColor target:(id)target action:(SEL)action{
