...
@interface LSTabBarController () <UIScrollViewDelegate>
...
- (instancetype)initWithControllerArray:(NSArray *)controllers titleArray:(NSArray *)titleArray imageArray:(NSArray *)imageArray selImageArray:(NSArray *)selImageArray{
    ...
    if (self = [super init]) {
        ...
        NSMutableArray *vcs = [[NSMutableArray alloc] initWithCapacity:20];
        ...
        for (int i = 0; i < controllers.count; i ++) {
            ...
            UIImage *selectImage = [UIImage imageNamed:selImageArray[i]];
            ...
            //[vcs addObject:[[WMNavigationController alloc] initWithRootViewController:vc]];
            [vcs addObject:vc];
        }
        
        self.tabBar.tintColor = [UIColor colorWithRed:38.0 / 255.0 green:143.0 / 255.0 blue:232.0 / 255.0 alpha:1];
...
- (void)viewDidLoad {
    ...
    [self.view addSubview:self.customerNavigation];
...
- (void)show {
    if ([self.selectedViewController respondsToSelector:@selector(refreshContent)]) {
        [self.selectedViewController performSelector:@selector(refreshContent) withObject:nil];
    }
    [self.navigationController setNavigationBarHidden:NO animated:YES];
}
- (void)hide {
...
- (void)setSelectedIndex:(NSUInteger)selectedIndex {
    ...
    if (![LocalStoreData getOnLine] && selectedIndex != 0) {
        [LocalToastView toastWithText:@"请开始作业！"];
        return;
    }
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    if (scrollView.contentOffset.x <= 10) {
        [self.customerNavigation reloadWithTitle:@"工作"];
    }
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:@"kNotificationCenterChangeState" object:nil];
    [[NSNotificationCenter defaultCenter] postNotificationName:@"kNotificationCenterFreshMyPage" object:nil];
}
...
- (CustomerNavigationView *)customerNavigation {
    if (_customerNavigation == nil) {
        _customerNavigation = [[CustomerNavigationView alloc] initWithFrame:CGRectMake(0, 0, kScreenWidth, 64)];
        _customerNavigation.backgroundColor = ThemeColor;
    }
