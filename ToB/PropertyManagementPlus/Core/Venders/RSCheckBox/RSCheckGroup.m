...
-(void)checkboxBtnChecked:(RSCheckBox *)sender
{
    ...
    if ([self.delegate respondsToSelector:@selector(clickWithItem:index:isAll:isSelected:sender:)]) {
        [self.delegate clickWithItem:sender.value/1000 index:sender.value%1000 isAll:([sender.titleLabel.text isEqualToString:@"全选"]) isSelected:sender.selected sender:sender];
    }
}

- (void)checkboxBtnActionChecked:(RSCheckBox *)sender {
